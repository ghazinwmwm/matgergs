...
import { Plus, Store, Settings, Users, Trash2, ChevronLeft, Globe, MapPin } from "lucide-react";
          ...
          {stores.map((store) => (
            <div key={store.id} className="bg-card border border-border rounded-xl p-4">
              ...
              <div className="flex gap-2 mt-3">
                ...
                <Button variant="outline" size="sm" className="flex-1 gap-1 text-xs">
                  <Users className="h-3 w-3" /> المديرين
                </Button>
                {stores.length > 1 && (
                  <Button variant="outline" size="sm" className="text-destructive hover:text-destructive gap-1 text-xs" onClick={() => deleteStore(store.id)}>
