...
import { Plus, Trash2, Copy, Code, Eye, Facebook, Instagram, Activity, BarChart3 } from "lucide-react";
...
const Tracking = () => {
  ...
  const [newPixel, setNewPixel] = useState({
    platform: "facebook" as Pixel["platform"],
    name: "",
    pixelId: "",
  });
  ...
  const addPixel = () => {
    ...
    const pixel: Pixel = {
      ...
      ...newPixel,
      name: newPixel.name || PLATFORM_INFO[newPixel.platform].label,
      active: true,
      events: ["PageView"],
    };
  ...
  const copyId = (id: string) => {
    ...
    toast({ title: "تم النسخ", description: "تم نسخ معرّف البيكسل" });
  };
      ...
      <main className="container mx-auto px-4 space-y-4">
        {showForm && (
          <div className="bg-card border border-border rounded-xl p-4 space-y-3 animate-slide-in">
                ...
                {(Object.keys(PLATFORM_INFO) as Pixel["platform"][]).map((platform) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1 py-2.5 rounded-lg text-[10px] font-medium border transition-colors ${
                        newPixel.platform === platform
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">معرّف البيكسل / Pixel ID</label>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-0.5">
                البيكسل هو كود تتبع يُضاف لمتجرك لمعرفة سلوك الزوار وقياس فعالية الإعلانات. يساعدك على استهداف العملاء بشكل أدق.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {pixels.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
              ...
              <p className="text-sm font-medium">لا يوجد بيكسل</p>
            </div>
          ) : (
            pixels.map((pixel) => {
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <button onClick={() => copyId(pixel.pixelId)} className="text-muted-foreground hover:text-foreground">
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
                    <div className="flex flex-wrap gap-1">
                      {pixel.events.map((event) => (
                        <span key={event} className="text-[10px] px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground font-mono">
                          {event}
                        </span>
                      ))}
                    </div>
                    <button onClick={() => deletePixel(pixel.id)} className="text-muted-foreground hover:text-destructive transition-colors">
